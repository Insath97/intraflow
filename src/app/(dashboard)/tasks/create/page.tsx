...
import { taskService, type TaskStatus, type TaskPriority, STATUS_COLORS, PRIORITY_COLORS } from "@/lib/api/tasks";
...
import {
  Plus, ArrowLeft, Loader2, ClipboardList, FileText, Settings, Users,
  Calendar, CheckCircle2, Trash2, GripVertical, Search, ChevronDown, X, Check,
} from "lucide-react";
...
function Combo({ value, onChange, items, placeholder, loading, renderLabel }: { value: string; onChange: (id: string) => void; items: DD[]; placeholder?: string; loading?: boolean; renderLabel?: (i: DD) => React.ReactNode; }) {
      ...
      <div className="max-h-60 overflow-y-auto p-1">
        ...
        {filtered.map((item) => (
          <button key={item.id} type="button" onClick={() => { onChange(value === item.id ? "" : item.id); setOpen(false); setSearch(""); }}
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">{renderLabel ? renderLabel(item) : <span className="truncate">{item.name}</span>}</div>
  ...
  return (
    <div ref={cRef} className="relative">
      <button type="button" onClick={() => { up(); setOpen(true); setTimeout(() => iRef.current?.focus(), 0); }}
        ...
        <span className={cn("truncate", !sel && (isDark ? "text-gray-500" : "text-gray-400"))}>{loading ? "Loading..." : sel ? (renderLabel ? renderLabel(sel) : sel.name) : (placeholder || "Select")}</span>
...
function UserCombo({ value, onChange, users, loading, placeholder }: { value: string; onChange: (id: string) => void; users: UserSimple[]; loading: boolean; placeholder?: string; }) {
  const isDark = useIsDark();
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const cRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={cRef} className="relative">
      <button type="button" onClick={() => { up(); setOpen(true); setTimeout(() => iRef.current?.focus(), 0); }}
        className={cn("flex h-10 w-full items-center justify-between rounded-lg border px-3 py-2 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00] focus-visible:ring-offset-1", isDark ? "border-white/10 bg-[#1A1D2E] text-gray-100" : "border-gray-200 bg-white text-gray-900", open && "ring-2 ring-[#FF6B00] ring-offset-1")}>
        <span className={cn("truncate", !sel && (isDark ? "text-gray-500" : "text-gray-400"))}>{loading ? "Loading..." : sel ? sel.full_name : (placeholder || "Select assignee")}</span>
        <div className="flex items-center gap-1 shrink-0 ml-1">
          {value && <span onClick={(e) => { e.stopPropagation(); onChange(""); }} className={cn("rounded-full p-0.5", isDark ? "text-gray-500 hover:text-gray-300 hover:bg-white/10" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100")}><X className="h-3.5 w-3.5" /></span>}
          <ChevronDown className={cn("h-4 w-4 transition-transform", isDark ? "text-gray-500" : "text-gray-400", open && "rotate-180")} />
        </div>
...
export default function CreateTaskPage() {
  ...
  const [form, setForm] = useState({
    project_id: "", module_id: "", title: "", description: "", assigned_to: "",
    priority: "MEDIUM" as TaskPriority, status: "TODO" as TaskStatus,
    start_date: "", due_date: "", estimated_hours: "", completion_pct: 0,
  });
  ...
  useEffect(() => {
    async function load() {
      try {
        const [pRes, uRes] = await Promise.all([projectService.dropdown(), usersApi.simple(true)]);
  ...
  async function handleSave() {
    ...
    try {
      const res = await taskService.create({
        ...
        subtasks: subtasks.length > 0 ? subtasks.map((t) => ({ title: t })) : undefined,
      });
      if (res.data.status === "success") { toast(res.data.message || "Task created", "success"); setTimeout(() => router.push("/tasks"), 500); }
      else toast(res.data.message || "Failed to create task", "error");
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Create Task" description="Add a new task to a project" breadcrumbs={[{ label: "Dashboard", onClick: () => router.push("/dashboard") }, { label: "Tasks", onClick: () => router.push("/tasks") }, { label: "Create" }]}
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="xl:col-span-2 space-y-6">
            ...
            <CardContent className="p-6">
              <div className="grid gap-5 sm:grid-cols-2">
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Project <span className="text-red-500">*</span></label>
                  <Combo value={form.project_id} onChange={(v) => setForm({ ...form, project_id: v, module_id: "" })} items={projects} placeholder="Select project" loading={loadingProjects} />
                ...
                <div className="sm:col-span-2"><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                  <textarea placeholder="Describe the task..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} className="flex w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00] focus-visible:ring-offset-1 dark:border-white/10 dark:bg-[#1A1D2E] dark:text-gray-100 dark:placeholder:text-gray-500" /></div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Schedule & Progress */}
          <Card className="overflow-hidden">
            <div className={sh}><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-green-100 text-green-600"><Calendar className="h-4.5 w-4.5" /></div><div><h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Schedule & Progress</h3><p className="text-xs text-gray-500 dark:text-gray-400">Dates and completion</p></div></div></div>
            <CardContent className="p-6">
              <div className="grid gap-5 sm:grid-cols-3">
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Start Date</label>
                  <Input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} /></div>
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Due Date</label>
                  <Input type="date" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} /></div>
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Est. Hours</label>
                  <Input type="number" placeholder="0" min="0" step="0.5" value={form.estimated_hours} onChange={(e) => setForm({ ...form, estimated_hours: e.target.value })} /></div>
                <div className="sm:col-span-3">
                  <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Completion: {form.completion_pct}%</label>
                  <input type="range" min="0" max="100" value={form.completion_pct} onChange={(e) => setForm({ ...form, completion_pct: parseInt(e.target.value) })} className="w-full accent-[#FF6B00]" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent className="p-6">
              <div className="flex gap-2 mb-4">
                <Input placeholder="Add a subtask..." value={newSubtask} onChange={(e) => setNewSubtask(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSubtask())} />
                <Button type="button" variant="outline" onClick={addSubtask}><Plus className="h-4 w-4" /></Button>
              </div>
              {subtasks.length === 0 ? (
                <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">No subtasks added yet</p>
              ) : (
                <div className="space-y-2">
                  {subtasks.map((st, i) => (
                    <div key={i} className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50/50 px-3 py-2 dark:border-white/5 dark:bg-white/[0.02]">
                      ...
                      <span className="flex-1 text-sm text-gray-700 dark:text-gray-300">{st}</span>
        ...
        <div className="xl:col-span-1 space-y-6">
          {/* Assignment */}
          <Card className="overflow-hidden">
            <div className={sh}><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-100 text-purple-600"><Users className="h-4.5 w-4.5" /></div><div><h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Assignment</h3><p className="text-xs text-gray-500 dark:text-gray-400">Who is responsible</p></div></div></div>
            <CardContent className="p-6 space-y-4">
              <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Assignee <span className="text-red-500">*</span></label>
                <UserCombo value={form.assigned_to} onChange={(v) => setForm({ ...form, assigned_to: v })} users={users} loading={loadingUsers} />
              ...
              <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Priority</label>
                <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value as TaskPriority })} className="flex h-10 w-full appearance-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-[#1A1D2E] dark:text-gray-100">
                  {ALL_PRIORITIES.map((p) => <option key={p} value={p}>{PL[p]}</option>)}
                </select></div>
              <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as TaskStatus })} className="flex h-10 w-full appearance-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-[#1A1D2E] dark:text-gray-100">
                  {ALL_STATUSES.map((s) => <option key={s} value={s}>{SL[s]}</option>)}
                </select></div>
            </CardContent>
          </Card>
          ...
          {/* Summary */}
          <Card className="overflow-hidden">
            <div className={sh}><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-100 text-amber-600"><Settings className="h-4.5 w-4.5" /></div><div><h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Summary</h3><p className="text-xs text-gray-500 dark:text-gray-400">Review before saving</p></div></div></div>
            <CardContent className="p-6">
