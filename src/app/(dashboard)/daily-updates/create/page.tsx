...
import { Plus, ArrowLeft, Loader2, Briefcase, Trash2, Search, ChevronDown, X, Check, Clock } from "lucide-react";
...
function Combo({ value, onChange, items, placeholder, loading, error }: { value: string; onChange: (id: string) => void; items: DD[]; placeholder?: string; loading?: boolean; error?: boolean }) {
  ...
  return (
    <div ref={cRef} className="relative">
      <button type="button" onClick={() => { up(); setOpen(true); setTimeout(() => iRef.current?.focus(), 0); }}
        className={cn("flex h-10 w-full items-center justify-between rounded-lg border px-3 py-2 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00] focus-visible:ring-offset-1", isDark ? "border-white/10 bg-[#1A1D2E] text-gray-100" : "border-gray-200 bg-white text-gray-900", error && "border-red-500", open && "ring-2 ring-[#FF6B00] ring-offset-1")}>
...
export default function CreateDailyUpdatePage() {
  ...
  useEffect(() => {
    async function load() {
      try { const res = await projectService.dropdown(); if (res.data.status === "success") setProjects(res.data.data); } catch { /* ignore */ }
  ...
  async function loadTasks(projectId: string) {
    ...
    try { const res = await taskService.getDropdown({ project_id: projectId }); if (res.data.status === "success") setTasksByProject((p) => ({ ...p, [projectId]: res.data.data.map((t) => ({ id: t.id, name: t.title })) })); } catch { /* ignore */ }
                  ...
                  <CardContent className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Combo value={entry.project_id} onChange={(v) => { updateEntry(idx, "project_id", v); if (v) loadTasks(v); }} items={projects} placeholder="Select project" loading={loadingProjects} error={!!errors[`entry_${idx}_project`]} />
                      ...
                      <div>
                        ...
                        <Combo value={entry.task_id} onChange={(v) => updateEntry(idx, "task_id", v)} items={entryTasks} placeholder="Select task" loading={isLoadingTasks} />
                      </div>
                    </div>
                    {/* Row 2: type, location, start, end */}
