...
export default function CreatePersonPage() {
  ...
  useEffect(() => {
    if (formData.dsDivisionId) {
      ...
      setFormData((prev) => ({ ...prev, gnDivisionId: "" }));
    } else {
      setGNDivisions([]);
    }
  ...
  const toggleAssistance = useCallback(
    ...
    [formData.assistanceRequired, updateField]
  );
  ...
  const toggleEquipment = useCallback(
    ...
    [formData.equipmentRequired, updateField]
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      AuditLogService.log(
        ...
        "Persons",
        person.id,
        `Registered person: ${person.fullName}`,
        "success",
        `Registration No: ${person.registrationNo}`,
        user?.id,
        user?.name
      );
      ...
      toast("Failed to register person. Please try again.", "error");
    } finally {
      setSubmitting(false);
    }
  }, [formData, currentStep, validateStep, toast, router, user]);

  const getProvinceName = (id: string) => provinces.find((p) => p.id === id)?.name || "";
  const getDistrictName = (id: string) => districts.find((d) => d.id === id)?.name || "";
  const getDSName = (id: string) => dsDivisions.find((d) => d.id === id)?.name || "";
  const getGNName = (id: string) => gnDivisions.find((g) => g.id === id)?.name || "";

  return (
    <div className="space-y-6">
      <PageHeader
        title="Register New Person"
        description="Complete all steps to register a new person with disabilities."
        breadcrumbs={[
